...
| Canvas rendering: sprites, metasprites (per-clip), tilemaps, text, rect fills, transform markers, instance gizmos, merged backgrounds, error placeholders, selection outline | world panel `canvas` + `loader` (worldlib compose / `build_draw_list`), and a **Design\|Live** switch on the world toolbar (Phase 3) | ✓ | ggo-ide has ONE world page: the IDE draws the world itself, and that drawing is the only view of it. **Design** is that page — the `canvas`/`loader` renderer, unchanged and still where every open lands. **Live** is a second view ggo-ide had no answer for: it boots the emerald viewer cart (`emd editor-cart --ggo`) in the emu panel and mirrors the open document to it over the link-cable protocol (`emerald-editor-link`), so the world is drawn by the renderer the game itself will use — with the editor schedule always running and the project's own systems switched on one at a time from the systems rail. `ctrl-alt-l` (Linux/Windows) / `cmd-alt-l` (macOS) toggles the mode, and shadows the global bindings on that chord only while the world panel has focus. The caveats are Live's, not Design's. A background stroke reaches the live view when the `.map` is SAVED — the layer payloads are read from disk, so the save is what makes the painted cells reachable (follow-up: send the paint session's in-memory payload during the stroke, for a live preview). The systems mask is panel-wide and sticky while system NAMES are per-cart, so bit N can mean two different systems across two projects in one session. A viewer-cart failure falls back to Design, ends the viewer run and puts the reason on the toolbar's error row — clicking **Live** again boots a fresh cart. MCP `world_open` and the close prompt's reload both land in Design, because a windowless load has no window to boot a cart in. Watch-mode rebuilds for a viewer run ignore `worlds/**.toml` and `maps/**.map`, since those documents travel over the link rather than being packed into the cart. |
