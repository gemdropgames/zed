...
| World page rendering (ggo-ide has one: the IDE draws the world itself, and that drawing is the only view of it) | The **Design\|Live** switch on the world toolbar (Phase 3): **Live** boots the emerald viewer cart (`emd editor-cart --ggo`) in the emu panel and mirrors the open document to it over the link-cable protocol (`emerald-editor-link`), so the world is drawn by the renderer the game itself will use — with the editor schedule always running and the project's own systems switched on one at a time from the systems rail; **Design** is the existing `canvas`/`loader` renderer, unchanged and still where every open lands | partial | Live painting re-sends the SAVED `.map`, so a stroke reaches the live view only once the map is saved (follow-up: send the paint session's in-memory layer payload instead). `ctrl-alt-l` (Linux/Windows) / `cmd-alt-l` (macOS) toggles the mode, and shadows the global bindings on that chord only while the world panel has focus. The systems mask is panel-wide and sticky while system NAMES are per-cart, so bit N can mean two different systems across two projects in one session. A viewer-cart failure falls back to Design and puts the reason on the toolbar's error row — clicking **Live** again retries. MCP `world_open` and the close prompt's reload both land in Design, because a windowless load has no window to boot a cart in. Watch-mode rebuilds for a viewer run ignore `worlds/**.toml` and `maps/**.map`, since those documents travel over the link rather than being packed into the cart. |
